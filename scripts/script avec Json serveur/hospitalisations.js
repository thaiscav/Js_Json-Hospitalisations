    ...
    xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                try {
                    ...
                    document.getElementById('container5-alert').innerHTML = e;
                }
                choisirTable();
            } //fin if check readystate e status
        } // fin fun onreadystatechange
    xmlhttp.open("GET", "scripts/data.json", true);
    xmlhttp.send();
} //fin func loadJSON

// Requisition XMLHttpRequest
function checkXmlHttp() {
    var xmlhttp;
    // verifie XMLHttpRequest
    if (window.XMLHttpRequest) {
        xmlhttp = new XMLHttpRequest();
    }
    else {
        // Add ActiveXObject de Microsoft 
        xmlhttp = new ActiveXObject('Microsoft.XMLHTTP');
    }
    return xmlhttp;
} //fin func checkXmlHttp
...
function chargerTabHosp(sel) {
        ...
        tab = dadosJSON.hospitalisations;
        ...
        for (j in tab) {
            ...
            else if ((tab[j].dossier === sel) && (list1==dadosJSON.patients)){
                 remplirTabHosp();
            }
            //choix menu HOSPIT par ETAB =>
            else if ((tab[j].codeEtab === sel) && (list1==dadosJSON.etablissements)){
                remplirTabHosp();
            }
...
function remplirListeEtab() {
        ...
        list1 = dadosJSON.etablissements;
...
function remplirListeSpecialite(sel) { //onchange Etab
        ...
        list2 = dadosJSON.hospitalisations;  
